...
from gbp_ps import add_or_update_process
...
def prepull_handler(*, build: Build) -> None:
    ...
    add_or_update_process(
        create_build_process(build, platform.node(), "pull", _now(tz=dt.UTC))
    )
...
def postpull_handler(*, build: Build, **_kwargs: Any) -> None:
    ...
    add_or_update_process(
        create_build_process(build, platform.node(), "clean", _now(tz=dt.UTC))
    )
